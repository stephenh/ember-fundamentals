...
if (require.entries['emberli/components/course-list-item']) {
  ...
  test('DOM structure is as expected', function(assert) {
    ...
    assert.equal($components.length, 1, 'Component has top-level class of .course-list-item');    
    let prop = $components[0].attributes.getNamedItem('data-name');
    assert.equal(prop ? prop.value : null, 'My Course', 'Course title can be found in component\'s boundary element, under DOM attribute "data-name"');
  ...
  test('Rendering performance optimizations', function(assert) {
    ...
    let $components = this.$().children('.course-list-item');
    assert.equal($components.find('.course-title').text().trim(), 'My Course', 'Course title does not update in response to data changes');
    assert.equal($components.find('.course-summary').text().trim(), 'A course description', 'Course summary does not update in response to data changes');
    assert.equal($components.find('img.course-image').attr('src'), 'abc', 'Course image does not update in response to data changes');
  });
}
